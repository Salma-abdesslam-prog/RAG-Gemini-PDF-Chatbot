...
def ensure_api_key() -> bool:
    ...
    if api_key:
        st.success("Authentification Gemini API prete.")
        return True

    entered_key = st.text_input(
        "Entrez votre GEMINI_API_KEY :",
        type="password",
        help="La cle est lue depuis GEMINI_API_KEY."
    )

    if not entered_key:
        st.error("Cle Gemini API requise.")
        return False

    # Rend la cle disponible au SDK google-genai.
...
def build_collection_from_pdf(pdf_bytes: bytes, model_name: str):
    ...
    try:
        ...
        with st.spinner("2/3 - Creation des chunks et de la base vectorielle..."):
...
uploaded_file = st.file_uploader("Deposez votre fichier PDF ici :", type=["pdf"])
user_prompt = st.chat_input("Posez votre question ici...")

if uploaded_file is None:
    st.info("Importez un PDF pour commencer.")
    st.stop()
...
try:
    ...
    st.error("Erreur pendant la preparation du document.")
    st.exception(e)
    st.stop()

if user_prompt:
    try:
        with st.spinner("3/3 - Recherche semantique en cours..."):
        ...
        with st.expander("Passages de contexte trouves :"):
        ...
        with st.spinner("Generation de la reponse avec Gemini..."):
        ...
        st.error("Erreur pendant la recherche ou la generation de reponse.")
        st.exception(e)
